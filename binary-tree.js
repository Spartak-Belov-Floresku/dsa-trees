...
class BinaryTree {
  ...
  /*maxSum() {
    ...
    const maxSumHelper = node => {
      ...
      if (!node) return 0;
      return Math.max(maxSumHelper(node.left)+node.val, maxSumHelper(node.right)+node.val);
  ...
  nextLarger(lowerBound) {
    ...
    const nextLargerHelper = node => {
      if(!node) return num;
      if(node.val > lowerBound) num = node.val;
      return Math.max(nextLargerHelper(node.left), nextLargerHelper(node.right));
    }
    return !nextLargerHelper(this.root)? null: num;
  }
